...
### Approximations in bootstrap

How does bootstrap work? Consider traditional bootstrap example [@Efron:1983bw]. The dataset is GPA scores of various entering classes at American law schools in 1973.
...
$$\mathbf{X}_1, \ldots, \mathbf{X}_n \iid F$$

where $F$ is true unknown distribution. Having observed $\mathbf{X}_1 = \mathbf{x}_1, \ldots, \mathbf{X}_n = \mathbf{x}_n$, we get *empirical distribution function* by computing the sample average.
...
This works for estimator of $F$. $\hat{F}$ endows mass $\frac{1}{n}$ on each observed point $\mathbf{x}_i, \: i = 1, \ldots, n$. In other words, pseudo-distribution becomes *discrete uniform*.
...
We have set the population distribution which is bogus. Now we can apply previous MC sampling with $\frac{1}{n}$. Write $\{ \mathbf{X}_1^{\ast}, \ldots, \mathbf{X}_n^{\ast} \}$ as *bootstrap sample* by resampling. Then
...
\begin{equation}
  \mathbf{X}^{\ast} \iid unif(\mathbf{x}_1, \ldots, \mathbf{x}_n)
  (\#eq:bootdistn)
\end{equation}
