...
with $\mathbf{P}_0 = \frac{1}{n}\mathbf{1} \in \R^n$. We already have seen that the bootstrap standard error is
...
where $J = [ 1 ] \in \R^{n \times n}$. In comparison, the jackknife resamples leaving out one observation so that
...
These approximations \@ref(lem:jackboot) and \@ref(thm:jackse) imply that the jackknife is almost a bootstrap itself [@Efron:1983bw].

```{theorem, jackse2, name = "Jackknife SE approximation"}
Linear approximation by jackknife \@ref(lem:jackboot) has a standard error

$$Var\hat\theta_L(\mathbf{P}^{\ast}) = Var(\mathbf{P}^{\ast} - \mathbf{P}_0)^T U = \frac{1}{n^2} \sum_{i = 1}^n U_i^2$$
```

```{proof}
Note that $\sum\limits_i U_i = 0$. From Equation \@ref(eq:multicov) and Lemma \@ref(lem:jackboot),
...
In Section \@ref(bootest), bootstrap estimates standard error and bias of any estimator using resampling, denoted by $\hat\sigma_B$ and $\hat\beta_B$. Note that these two are random variables, too. We could wonder the variance of these estimators. For this, we use *Jackknife*.

```{r jabfig, echo=FALSE, fig.cap="Jackknife-after-bootstrap"}
knitr::include_graphics("images/jackafterboot.png")
```
...
```{r quanfig, echo=FALSE, fig.cap="$P(\\hat\\theta - \\theta > \\alpha_{\\frac{\\alpha}{2}}) = 1 - \\frac{\\alpha}{2}$"}
...
### Percentile bootstrap confidence interval {#percboot}
...
We have used normal distribution so far, but this might not exactly correct. In many cases, Student $t$ distribution gives more reasonable standard error. Here, bootstrap does not use Student $t$. Instead, the sampling distribution of a $t$-type statistic is generated by resampling [@Rizzo:2007aa].

```{theorem, boottci, name = "Bootstrap t confidence interval"}
Let $t^{\ast}_{\frac{\alpha}{2}}$ and $t^{\ast}_{1 - \frac{\alpha}{2}}$ be $\frac{\alpha}{2}$ and $1 - \frac{\alpha}{2}$ quantile values of bootstrap replicates of $t$-type statistic.
...
Better bootstrap confidence interval is also called **BCa** for biased corrected and adjusted for acceleration. Consider percentile bootstrap CI \@ref(thm:percci) in Section \@ref(percboot). @Efron:1983bw expresses this in terms of an **putative central CI**.

```{theorem, medci, name = "Putative central confidence interval"}
Let
...
Before computing CI, we need to estimate these $z_0$ and $a$. Since we do not know $g$, we use $\Phi(z)$, the cdf of standard normal with $\Phi(z_\alpha) = \alpha$ [@Rizzo:2007aa]. We can check that above percentile CI results in the similar values to the standard normal bootstrap CI, by central limit theorem. Thus, we would use $\Phi^{-1}$ when estimating the bias.

```{theorem, estz0bca, name = "Estimation of bias correction factor"}
Bias can be corrected by $t = \hat\theta$, i.e.
...
```{theorem, estabca, name = "Estimation of acceleration factor"}
Acceleration factor is estimated from \textit{Jackknife replicates}
...
From Equation \@ref(eq:percciadj), our goal is to get for each $z = z_{\frac{\alpha}{2}}, z_{1 - \frac{\alpha}{2}}$,
...
```{theorem, bcaci, name = "BCa confidence interval"}
Let $\hat{z}_0$ be bias correction factor estimated by Thoerem \@ref(thm:estz0bca) and let $\hat{a}$ be acceleration factor estimated by Theorem \@ref(thm:estabca). Compute
